...
//ambil bulan dari tanggal penjualan terakhir
 ...
 $bulan_terakhir = $db->query("SELECT MONTH(tanggal) as bulan FROM penarikan ORDER BY id DESC LIMIT 1");
 $v_bulan_terakhir = mysqli_fetch_array($bulan_terakhir);

//ambil nomor  dari penjualan terakhir
$no_terakhir = $db->query("SELECT no_faktur FROM penarikan ORDER BY id DESC LIMIT 1");
 ...
 */
 if ($v_bulan_terakhir['bulan'] != $bulan_sekarang) {
  # code...
echo$no_faktur = "1/PA/".$data_bulan_terakhir."/".$tahun_terakhir;
...
echo$no_faktur = $nomor."/PA/".$data_bulan_terakhir."/".$tahun_terakhir;


 }

// buat prepared statements
    $stmt = $db->prepare("INSERT INTO penarikan (no_faktur,dari_akun,jumlah,tanggal,jam,user) VALUES (?,?,?,?,?,?)");
...
if (!$stmt) {
   ...
   $query1 = $db->query("SELECT * FROM tbs_penarikan WHERE session_id = '$session_id'");

    while ($data=mysqli_fetch_array($query1)) {

    $query2 = $db->query("INSERT INTO detail_penarikan (no_faktur,keterangan,dari_akun,ke_akun,jumlah,tanggal,jam,user,jurusan) VALUES ('$no_faktur','$data[keterangan]','$data[dari_akun]','$data[ke_akun]','$data[jumlah]','$data[tanggal]','$data[jam]','$data[user]','$data[jurusan]')");
...
/*
    $ambil_tbs = $db->query("SELECT * FROM tbs_penarikan WHERE session_id = '$session_id'");
...

{
  ...
  */
    $query3 = $db->query("DELETE FROM tbs_penarikan WHERE session_id = '$session_id'");                      
